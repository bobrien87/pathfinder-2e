...
armorclass:
  - name: AC
    desc: "22; **Fort** +16, **Ref** +11, **Will** +13"
health:
  - name: HP
    desc: "105; (4 segments); Thresholds 70 (3 segments), 35 (2 segments); **Weaknesses** area damage 5, splash damage 5"
abilities_mid:
...
abilities_bot:
  - name: "Bombing Barrage"
    desc: "`pf2:2` The dwarf battalion draws alchemical bombs, then hurls them at distant foes. This volley is a @Template[type:burst|distance:10] within 60 feet that deals @Damage[3d6[@item.flags.system.rulesSelections.bombingBarrage]|options:area-damage] damage with a DC 21 [[Reflex]] save. The damage is either acid, fire, or electricity damage, depending on which type of bombs the battalion used. When the dwarf battalion is reduced to 2 or fewer segments, this area decreases to a @Template[type:burst|distance:5]."
  - name: "Coordinated Pummel"
    desc: "`pf2:1` `pf2:1` to `pf2:3` <br>  <br> **Frequency** once per round <br>  <br> **Effect** The dwarf battalion unleashes a storm of warhammer blows against each enemy in a @Template[type:emanation|distance:5] (DC 21 [[Reflex]] save). The damage depends on the number of actions. <br>  <br> `pf2:1` @Damage[1d8[bludgeoning]|options:area-damage] damage <br>  <br> `pf2:2` @Damage[(2d8+5)[bludgeoning]|options:area-damage] damage <br>  <br> `pf2:3` @Damage[(3d8+5)[bludgeoning]|options:area-damage] damage"
  - name: "Dwarven War Song"
    desc: "`pf2:1` The battalion joins together to sing a traditional song of battle. Each enemy in a @Template[type:emanation|distance:30] must succeed at a DC 23 [[Will]] save or be [[Frightened]] 1 (or [[Frightened]] 2 on a critical failure). Each enemy is then temporarily immune for 10 minutes."
sourcebook: "NPC Core"
